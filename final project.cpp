...
//Functions used in patient management
    ...
    for (int i = 0; i < num; i++) {
        if (name == pname[i]) {
            ...
            string newName;
            getline(cin, newName);
            if (!newName.empty()) pname[i] = newName;

            cout << "Gender (1. Male, 2. Female) [" << gender[i] << "]: ";
            int newGender;
            cin >> newGender;
            cin.ignore();  // Clear input buffer
            if (newGender == 1) gender[i] = "Male";
            else if (newGender == 2) gender[i] = "Female";

            cout << "ID [" << pid[i] << "]: ";
            string newID;
            getline(cin, newID);
            if (!newID.empty()) pid[i] = stoi(newID);

            cout << "Age [" << age[i] << "]: ";
            int newAge;
            cin >> newAge;
            cin.ignore();
            if (newAge > 0) age[i] = newAge;

            cout << "Weight [" << weight[i] << "]: ";
            double newWeight;
            cin >> newWeight;
            cin.ignore();
            if (newWeight > 0) weight[i] = newWeight;

            cout << "Height [" << height[i] << "]: ";
            double newHeight;
            cin >> newHeight;
            cin.ignore();
            if (newHeight > 0) height[i] = newHeight;

            cout << "Medical History [" << p_discription[i] << "]: ";
            string newHistory;
            getline(cin, newHistory);
            if (!newHistory.empty()) p_discription[i] = newHistory;
...
// Function to display the main menu
    ...
    int main()
    {
        ...
        cout <<"\n\n\t\t\t*******************************"
             ...
             <<"\n\n\t\t\t*******************************";
        cout <<"\n\n\t\tPlease register, then log in to continue";
